...
const activeldolBadge = document.getElementById('active-idol-badge');
...
const toolFrame = document.getElementById('tool-frame'); // ★追加: フレームツール
const toolClear = document.getElementById('tool-clear');
const modalOverlay = document.getElementById('modal-overlay');
const modalBody = document.getElementById('modal-body');
const modalCloseBtn = document.getElementById('modal-close-btn');

// ★ フレーム管理リスト (新しい画像フレームが増えたらここに追加)
const frameList = [
...
// ★ ストレージデータ取得 (サンプルグループの初期表示を削除)
let groups = JSON.parse(localStorage.getItem('cheki_groups')) || [];
...
let activeldolld = Number(localStorage.getItem('cheki_active_idol_id')) || (idolList.length > 0 ? idolList[0].id : null);
...
function getActiveldol() {
  return idolList.find(item => item.id === activeldolld) || { group: '', idol: '', color: '#FFFFFF' };
}

let currentColor = getActiveldol().color;

function updateActiveBadge() {
  const active = getActiveldol();
  if (active && (active.group || active.idol)) {
    activeldolBadge.textContent = `選択: ${active.group}/${active.idol}`;
    activeldolBadge.style.borderColor = active.color;
  } else {
    activeldolBadge.textContent = '選択: なし (タップして設定)';
    activeldolBadge.style.borderColor = 'rgba(255,255,255,0.3)';
  }
}
updateActiveBadge();
...
function saveData() {
  ...
  localStorage.setItem('cheki_active_idol_id', activeldolld);
...
function redrawCanvas() {
  ...
  // 1. チェキフレーム(メンカラ または 画像フレーム)
  if (selectedFrame && selectedFrame.type === 'image' && loadedFrameImages[selectedFrame.id]) {
    // 画像フレーム指定時: 背景は白で塗りつぶし
  ...
  // 2.5 画像フレームの重ね描画
  if (selectedFrame && selectedFrame.type === 'image' && loadedFrameImages[selectedFrame.id]) {
    ctx.drawImage(loadedFrameImages[selectedFrame.id], 0, 0, canvas.width, canvas.height);
  }
...
// 設定・メンバー追加
activeldolBadge.addEventListener('click', openSettingsModal);
...
function openSettingsModal() {
  ...
  let html = `
    ...
    <div class="setting-section">
      ...
      <div id="new-group-box" style="${groups.length === 0 ? 'display:block;' : 'display:none;'}">
  ...
  document.getElementById('add-idol-btn').addEventListener('click', () => {
    ...
    if (groupSelect.value === '_NEW_' || groups.length === 0) {
...
function renderIdolItemsInModal() {
  ...
  container.innerHTML = filtered.map(item =>
    `<div class="idol-select-item" style="padding:8px; border-left:4px solid ${item.color}; background:${item.id === activeldolld ? '#444' : '#2a2a2a'}; margin-bottom:4px; border-radius:4px; cursor:pointer; display: flex; justify-content:space-between; align-items:center;" data-id="${item.id}">
  ...
  container.querySelectorAll('.idol-select-item').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      activeldolld = Number(el.getAttribute('data-id'));
      currentColor = getActiveldol().color;
      saveData();
      updateActiveBadge();
  ...
  container.querySelectorAll('.del-idol-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (activeldolld === id) activeldolld = idolList.length > 0 ? idolList[0].id : null;
      saveData();
      renderIdolItemsInModal();
      updateActiveBadge();
    });
...
if (toolFrame) {
  toolFrame.addEventListener('click', () => {
    isEraser = false; isStampMode = false; selectedStampIndex = -1; redrawCanvas();
    updateActiveTool(toolFrame);
    let html = '<p style="text-align:center; font-weight:bold; margin-bottom:12px;">フレームデザインを選択</p><div style="display:grid; grid-template-columns: repeat(2, 1fr); gap:10px;">';
    frameList.forEach(f => {
...
function updateActiveTool(activeBtn) {
  [toolColor, toolSize, toolEraser, toolStamp, toolFrame].forEach(btn => btn && btn.classList.remove('active-tool'));
